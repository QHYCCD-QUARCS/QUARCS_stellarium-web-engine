...
import websocketConnectionMixin from '@/mixins/websocketConnectionMixin';
import websocketMessageMixin from '@/mixins/websocketMessageMixin';
import { ensureUnifiedRuntime, recordClientCommand, recordClientLog, setUnifiedWebSocketState } from '@/runtime/unifiedRuntime';
import {
  emitCaptureTrace,
  ...
  sendCaptureTraceLog,
  startCaptureTrace,
} from '@/domain/capture/captureTrace';
import {
  buildHistogramSnapshotKey,
  combineWhiteBalanceGains,
  createHistogramSnapshot,
  deriveTileGpmTransition,
  normalizeGain,
} from '@/domain/capture/imagePipelineState';
import {
  CAMERA_MUTEX_ROLE_TYPES,
  ...
  normalizeDriverKey,
} from '@/domain/devices/cameraModePolicy';
import { drainDeviceConfigChanges, queueDeviceConfigChange } from '@/domain/devices/deviceSettingsPersistence';
...
export default {
  mixins: [websocketConnectionMixin, websocketMessageMixin],
  data(context) {
    return {
      ...
      pendingDeviceConfigChanges: {},
      isFlushingDeviceConfigChanges: false,
  ...
  methods: {
    ...
    normalizeCaptureTraceValue(value) {
      return normalizeCaptureTraceValue(value);
    },
    sendCaptureTraceLog(parts, type = 'info') {
      return sendCaptureTraceLog(this, parts, type);
    },
    startCaptureTrace(payload = {}) {
      return startCaptureTrace(this, payload);
    },
    ensureCaptureTraceForCommand(message) {
      return ensureCaptureTraceForCommand(this, message);
    },
    emitCaptureTrace(stage, extra = {}, type = 'info') {
      return emitCaptureTrace(this, stage, extra, type);
    },
    handleBackendCaptureTrace(parts) {
      return handleBackendCaptureTrace(this, parts);
    },
    ...
    getPreWhiteBalanceGains() {
      return {
        gainR: normalizeGain(this.ImageGainR),
        gainB: normalizeGain(this.ImageGainB),
      };
    },
    getPostWhiteBalanceGains() {
      return {
        gainR: normalizeGain(this.postGainR),
        gainB: normalizeGain(this.postGainB),
      };
    },
    getFinalWhiteBalanceGains() {
      return combineWhiteBalanceGains(this.ImageGainR, this.ImageGainB, this.postGainR, this.postGainB);
    },
    ...
    buildHistogramSnapshotKey(sessionId = this.tileSessionId, frameId = this.tileFrameId) {
      return buildHistogramSnapshotKey(sessionId, frameId);
    },
    cacheAndEmitHistogram(histogram, options = {}) {
      ...
      const frameId = options.frameId != null ? options.frameId : this.tileFrameId;
      this.tileHistogram = createHistogramSnapshot(histogram, {
        sessionId,
        frameId,
    ...
    linkedCameraModeDevices(deviceType) {
      return linkedCameraModeDevices(this.devices, deviceType);
    },
    ...
    anySdkConnectedInCameraModeGroup(deviceType) {
      return this.linkedCameraModeDevices(deviceType).some(d =>
        d && d.isConnected && connectionModeOf(d) === 'SDK');
    },
    anyIndiConnectedInCameraModeGroup(deviceType) {
      return this.linkedCameraModeDevices(deviceType).some(d =>
        d && d.isConnected && connectionModeOf(d) === 'INDI');
    },
    ensureCameraModeConsistency(preferDeviceType = '') {
      cameraModeConsistencyActions(this.devices, preferDeviceType, this.selectedConnectionMode)
        .forEach(action => {
          this.requestSetConnectionMode(action.driverType, action.mode, { silent: true, fromPeer: true });
        });
    },
    ...
    getDeviceDisplayText(device) {
      return deviceDisplayText(device);
    },
    ...
    confirmDriver() {
      ...
      const targetSlot = Number.parseInt(String(currentDevice && currentDevice.ListNum), 10);
      if (!Number.isInteger(targetSlot)) {
        this.SendConsoleLogMsg(`Confirm Indi Driver failed: slot not found for ${this.CurrentDriverType}`, 'error');
        this.callShowMessageBox('Unable to select driver: device slot not found', 'error');
        return;
      }
      // 显式携带角色槽位，避免 Clear Driver 将后端 currentDeviceCode 复位后，
      // 再次确认驱动时写不到 Guider/MainCamera 等固定角色。
      this.$bus.$emit(
        'AppSendMessage',
        'Vue_Command',
        `ConfirmIndiDriver:${this.selectedDriver}:${this.BaudRateSelected}:${targetSlot}`
      );
    ...
    async handleTileGPM(gpm) {
      ...
      const transition = deriveTileGpmTransition({
        gpm: prevGpm,
        sessionId: this.tileSessionId,
        frameId: this.tileFrameId,
        imageWidth: this.showImageSizeX,
        imageHeight: this.showImageSizeY,
        levelMode: prevLevelMode,
      }, gpm);
      const {
        incomingFrameId,
        isNewSession,
    ...
    connectDriver() {
      ...
      const selectedCandidate = (this.inlineCameraAllocationCandidates || []).find(candidate =>
      ...
      this.devices.forEach(device => {
        if (device.driverType === driverType) {
          ...
          if (driverName && upperName !== 'SDK' && upperName !== 'INDI') {
            this.selectedDriver = driverSelectionValue(driverName);
          }
        }
      });
  ...
  showRoiImage(fileName, destX, destY, roiCfa = null) {
    ...
    queueDeviceConfigChange(label, value, driverType = '') {
      this.pendingDeviceConfigChanges = queueDeviceConfigChange(this.pendingDeviceConfigChanges, {
        label,
        value,
        driverType: driverType || this.CurrentDriverType || '',
      });
    },
    flushPendingDeviceConfigChanges(reason = 'device-settings-close') {
      if (this.isFlushingDeviceConfigChanges) return 0;
      const drained = drainDeviceConfigChanges(this.pendingDeviceConfigChanges);
      this.pendingDeviceConfigChanges = drained.pendingChanges;
      if (!drained.changes.length) return 0;

      this.isFlushingDeviceConfigChanges = true;
      try {
        drained.changes.forEach(change => {
          this.handleConfigChange(change.label, change.value, change.driverType, { skipDirty: true });
        });
        this.SendConsoleLogMsg(`Device settings persisted on ${reason}: ${drained.changes.length} item(s)`, 'info');
      } finally {
        this.isFlushingDeviceConfigChanges = false;
      }
      return drained.changes.length;
    },
    // 通用的配置更改处理函数：变更时立即发送，菜单关闭时重放最终值作为持久化兜底。
    handleConfigChange(label, value, driverType = '', options = {}) {
      if (this.isCurrentDeviceUnbound) return;
  ...
  watch: {
    drawer_2: function (isOpen, wasOpen) {
      if (wasOpen && !isOpen) {
        this.flushPendingDeviceConfigChanges('device-settings-close');
      }
    },
...
/* Inline camera 连接按钮 */
.inline-camera-allocation__connect-row {
  display: flex;
